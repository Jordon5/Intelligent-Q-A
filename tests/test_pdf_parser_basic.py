...
def test_parser_initialization():
    ...
    try:
        ...
        print("[OK] PDF 解析器初始化成功")
        return True
    except Exception as e:
        print("[ERROR] 解析器初始化失败:", e)
        return False
...
def test_visual_block_extraction():
    ...
    try:
        ...
        if hasattr(parser, '_extract_visual_blocks'):
            ...
            print("[ERROR] 视觉块提取方法不存在")
        
        # 测试多栏处理方法的存在
        if hasattr(parser, '_detect_columns'):
            print("[OK] 多栏检测方法存在")
        else:
            print("[ERROR] 多栏检测方法不存在")
        
        # 测试表格提取方法的存在
        if hasattr(parser, '_extract_table'):
            print("[OK] 表格提取方法存在")
        else:
            print("[ERROR] 表格提取方法不存在")
        
        # 测试图片描述方法的存在
        if hasattr(parser, '_describe_image'):
            print("[OK] 图片描述方法存在")
        else:
            print("[ERROR] 图片描述方法不存在")
        
        return True
    except Exception as e:
        print("[ERROR] 测试功能存在性失败:", e)
        return False
