...
### 1. ETL 流水线设计
- **文档解析模块**：使用 PyMuPDF 提取文本块及其坐标信息
- **多栏排版处理**：基于 x 坐标聚类检测多栏布局，按阅读顺序重组文本
- **表格提取**：支持 LlamaParse API（需配置 API Key）或 PyMuPDF 内置方法
- **图片处理**：支持 Qwen-VL/GPT-4o 多模态模型生成图片描述（需配置 API Key）

### 2. 混合检索策略
- **BM25 关键词检索**：使用 rank_bm25 库实现，对专有名词有精确匹配效果
- **Dense Vector 语义检索**：使用 ChromaDB 默认嵌入模型，对语义相关内容有良好理解
- **倒数排名融合算法**：对双路召回结果进行加权排序，提升检索准确率

### 3. 父子索引分块策略
- **小区块保证检索精度**：子块默认 600 字符，更容易与查询匹配
- **大区块提供完整上下文**：父块默认 2000 字符，提供完整信息
- **ID 关联实现父子映射**：通过唯一 ID 实现父子块的映射关系

### 4. 异步任务队列系统
- **FastAPI 异步服务**：异步处理 HTTP 请求，快速响应
- **Celery + Redis 任务队列**：解耦文件上传与处理任务
- **断点续传支持**：大文件分块上传，支持上传中断恢复
- **任务状态轮询**：实时查询任务处理进度
...
# 可选：高级 PDF 解析功能
# LLAMA_CLOUD_API_KEY=your-llama-cloud-api-key  # 表格提取
# OPENAI_API_KEY=your-openai-api-key            # 图片描述（备选）
```

> **注意**：
> - `QWEN_API_KEY` 是必需的，用于 LLM 对话功能
> - `LLAMA_CLOUD_API_KEY` 是可选的，用于高级表格提取（无此 Key 将使用 PyMuPDF 内置方法）
> - `OPENAI_API_KEY` 是可选的，用于图片描述（无此 Key 将返回占位符）
